...
var transitionFade = function transitionFade(host, child, speed, keepchildren) {
  return new Promise(function (resolve) {
    ...
    animateOpacity(1, 0, elements.currentContainer, speed).then(function () {
      !elements.current ? undefined : elements.current.setAttribute("slot", "hidden"); // animateOpacity(0, 1, elements.nextContainer, speed)

      requestAnimationFrame(function () {
        animateOpacity(0, 1, elements.currentContainer, speed);
      });
    });
    animateHeight(startHeight, elements.child.offsetHeight, elements.root, speed).then(function () {
      if (!keepchildren) {
