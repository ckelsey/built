import { FindElementIn, ValidateHtml, ObserveEvent, ReplaceElementContents } from '../..'
...
export const toggleOptions = (host, show) => {
    ...
    const options = Array.from(FindElementIn(overlay, `.select-option`, true))
    let selectedOption
...
export const filter = (host, value) => {
    ...
    const options = Array.from(FindElementIn(host.elements.overlay, `.select-option`, true))
